...
class FlightDirectTest < Minitest::Test
  def test_direct_list_flight
    setup = flight_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-flight", setup[:live] ? "live" : "unit")
    if _should_skip
...
def flight_direct_setup(mockres)
