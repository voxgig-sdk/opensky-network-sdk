...
class StateVectorDirectTest < Minitest::Test
  def test_direct_list_state_vector
    setup = state_vector_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-state_vector", setup[:live] ? "live" : "unit")
    if _should_skip
...
def state_vector_direct_setup(mockres)
