...
require_relative 'OpenskyNetwork_types'
...
class OpenskyNetworkSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = OpenskyNetworkHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = OpenskyNetworkHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, OpenskyNetworkFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    OpenskyNetworkUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = OpenskyNetworkHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = OpenskyNetworkHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = OpenskyNetworkHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    rescue OpenskyNetworkError => err
    ...
    if fetched.is_a?(Hash)
      status = OpenskyNetworkHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = OpenskyNetworkError.new(
        "graphql_error", "OpenskyNetworkSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = OpenskyNetworkSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
