...
class StateVectorEntity
  ...
  def make
    opts = @_entopts.dup
    StateVectorEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = OpenskyNetworkHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [StateVector, Hash] the current StateVector data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = OpenskyNetworkHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of StateVector fields)
  def match_get
  ...
  #
  # @param reqmatch [StateVectorListMatch, Hash, nil] match filter (any subset of
  #   StateVector fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<StateVector>, Array] the matching StateVector items; raises OpenskyNetworkError on failure
