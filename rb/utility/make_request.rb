...
module OpenskyNetworkUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = OpenskyNetworkResponse.new({})
    ...
    if fetch_err
      ...
      response = OpenskyNetworkResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = OpenskyNetworkResponse.new(fetched)
    else
