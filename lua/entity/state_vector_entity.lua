...
local StateVectorEntity = {}
StateVectorEntity.__index = StateVectorEntity


function StateVectorEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, StateVectorEntity)
  self._name = "state_vector"
  self._client = client
...
function StateVectorEntity:get_name()
  return self._name
end


function StateVectorEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return StateVectorEntity.new(self._client, opts)
end
...
function StateVectorEntity:mark_deleted()
  self._deleted = true
end


function StateVectorEntity:deleted()
  return true == self._deleted
end


function StateVectorEntity:data_set(args)
  if args ~= nil then
...
function StateVectorEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function StateVectorEntity:match_set(args)
  if args ~= nil then
...
function StateVectorEntity:match_get()
...
function StateVectorEntity:stream(action, args, callopts)
...
function StateVectorEntity:_run_op(ctx, post_done)
  local utility = self._utility
