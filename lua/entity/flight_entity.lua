...
local FlightEntity = {}
FlightEntity.__index = FlightEntity


function FlightEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, FlightEntity)
  self._name = "flight"
  self._client = client
...
function FlightEntity:get_name()
  return self._name
end


function FlightEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return FlightEntity.new(self._client, opts)
end
...
function FlightEntity:mark_deleted()
  self._deleted = true
end


function FlightEntity:deleted()
  return true == self._deleted
end


function FlightEntity:data_set(args)
  if args ~= nil then
...
function FlightEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function FlightEntity:match_set(args)
  if args ~= nil then
...
function FlightEntity:match_get()
...
function FlightEntity:stream(action, args, callopts)
...
---@param reqmatch FlightListMatch
---@param ctrl? table
---@return Flight[]
---@return string? err
function FlightEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function FlightEntity:_run_op(ctx, post_done)
  local utility = self._utility
