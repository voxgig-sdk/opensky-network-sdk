...
local OpenskyNetworkSDK = {}
OpenskyNetworkSDK.__index = OpenskyNetworkSDK
...
OpenskyNetworkSDK._make_feature = _make_feature


function OpenskyNetworkSDK.new(options)
  local self = setmetatable({}, OpenskyNetworkSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function OpenskyNetworkSDK:options_map()
...
function OpenskyNetworkSDK:get_utility()
  return Utility.copy(self._utility)
end


function OpenskyNetworkSDK:get_root_ctx()
  return self._rootctx
end


function OpenskyNetworkSDK:prepare(fetchargs)
...
function OpenskyNetworkSDK:direct(fetchargs)
...
-- by passing it.
function OpenskyNetworkSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function OpenskyNetworkSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "OpenskyNetworkSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function OpenskyNetworkSDK:Flight(data)
  ...
  if data == nil then
    if self._flight == nil then
      self._flight = EntityMod.new(self, nil)
    end
    return self._flight
  end
...
function OpenskyNetworkSDK:StateVector(data)
  ...
  if data == nil then
    if self._state_vector == nil then
      self._state_vector = EntityMod.new(self, nil)
    end
...
function OpenskyNetworkSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = OpenskyNetworkSDK.new(sdkopts)
  sdk.mode = "test"
