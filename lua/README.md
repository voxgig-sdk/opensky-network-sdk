# OpenskyNetwork Lua SDK



The Lua SDK for the OpenskyNetwork API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:Flight()` — each with the same small set of operations (`list`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
GitHub release tag (`lua/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/opensky-network-sdk/releases)),
...
| `path` | `table` | Array of waypoints representing the aircraft trajectory |
...
```
lua/
├── opensky-network_sdk.lua    -- Main SDK module
...
The main module (`opensky-network_sdk`) exports the SDK constructor
