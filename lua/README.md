...
The Lua SDK for the OpenskyNetwork API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Flight(nil):list(nil, nil)
...
local result, err = client:OpenskyNetwork(nil):load(
  { id = "test01" }, nil
)
