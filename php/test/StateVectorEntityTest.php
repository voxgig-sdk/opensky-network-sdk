...
class StateVectorEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = OpenskyNetworkSDK::test(null, null);
        $ent = $testsdk->StateVector(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->StateVector(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->StateVector(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = state_vector_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "state_vector." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set OPENSKY_NETWORK_TEST_STATE_VECTOR_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $state_vector_ref01_data = null;
        if (count($state_vector_ref01_data_raw) > 0) {
...
function state_vector_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/state_vector/StateVectorTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["state_vector01", "state_vector02", "state_vector03"] as $k) {
    ...
    $entid_env_raw = getenv("OPENSKY_NETWORK_TEST_STATE_VECTOR_ENTID");
