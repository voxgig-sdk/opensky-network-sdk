...
class StateVectorEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new StateVectorEntity($this->_client, $opts);
    }

    /**
     * @param StateVector|array $args StateVector data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = OpenskyNetworkHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return StateVector|array The current StateVector data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of StateVector fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = OpenskyNetworkHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of StateVector fields).
     */
    ...
    /**
     * List StateVector items matching the given filter.
     *
     * @param StateVectorListMatch|array|null $reqmatch Match filter (any subset
     *   of StateVector fields) as an assoc-array; StateVectorListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return StateVector[]|array A list of StateVector items as assoc-arrays at
     *   the SDK boundary; throws OpenskyNetworkError on failure (item-5 convention).
     */
