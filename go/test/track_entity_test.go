...
func TestTrackEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Track(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Track(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set OPENSKY_NETWORK_TEST_TRACK_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		trackRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.track")))
...
func trackBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("OPENSKY_NETWORK_TEST_TRACK_ENTID")
	...
	idmapResolved := core.ToMapAny(env["OPENSKY_NETWORK_TEST_TRACK_ENTID"])
