...
import (
	...
	omni "github.com/voxgig-sdk/opensky-network-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.OpenskyNetworkError); is {
				out := map[string]any{
					"name":    "OpenskyNetworkError",
...
func omniCtx(arg any, client *sdk.OpenskyNetworkSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.OpenskyNetworkSDK); is {
			client = live
