...
[MCP](https://modelcontextprotocol.io) server exposing the OpenskyNetwork SDK as
two agent tools — `opensky-network_list` and `opensky-network_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/opensky-network-mcp)
make build

# 2. Provide credentials via the environment
export OPENSKY_NETWORK_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user opensky-network \
  -- /absolute/path/to/opensky-network-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./opensky-network-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// opensky-network_list: first page of records
{ "entity": "flight" }
{ "entity": "flight", "query": { } }

// opensky-network_load: one record by id
{ "entity": "flight", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/opensky-network-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user opensky-network \
     -- "$PWD"/dist/*/opensky-network-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `opensky-network_list` and `opensky-network_load` tools now appear
   in new sessions. Ask the agent to *"list flight using opensky-network"*
   and it calls `opensky-network_list` with `{"entity":"flight"}`.
...
```sh
export OPENSKY_NETWORK_APIKEY=sk_live_xxx            # API key
export OPENSKY_NETWORK_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `opensky-network_list` tool
...
### Call the `opensky-network_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `opensky-network_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `opensky-network_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `OPENSKY_NETWORK_APIKEY` | API key sent with every request. |
| `OPENSKY_NETWORK_BASE` | Optional override of the API base URL. |
...
```sh
./opensky-network-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"opensky-network_load","arguments":{"entity":"flight","query":{"id":1}}}}'
```
