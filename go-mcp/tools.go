...
type Args struct {
	Entity string         `json:"entity" jsonschema:"flight | state_vector | track"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.OpenskyNetworkSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "opensky-network_list",
		Description: "List records from OpenskyNetwork. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "opensky-network_load",
		Description: "Load a single record from OpenskyNetwork. " +
...
func runOp(client *sdk.OpenskyNetworkSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.OpenskyNetworkSDK, name string) (sdk.OpenskyNetworkEntity, error) {
	switch strings.ToLower(name) {
	case "flight":
		return client.Flight(nil), nil
	case "state_vector":
		return client.StateVector(nil), nil
	case "track":
