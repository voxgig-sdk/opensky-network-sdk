// Command OpenskyNetwork-mcp is an MCP server exposing the OpenskyNetwork
// SDK as MCP tools.
//
// Two tools are registered:
//
//	OpenskyNetwork_list  — list records of an entity
//	OpenskyNetwork_load  — load a single record of an entity
//
...
func main() {
	...
	// Configure from the environment: OPENSKY_NETWORK_APIKEY carries the API key and
	// OPENSKY_NETWORK_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("OPENSKY_NETWORK_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("OPENSKY_NETWORK_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewOpenskyNetworkSDK(opts)
