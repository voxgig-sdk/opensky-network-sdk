...
The Python SDK for the OpenskyNetwork API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Flight(None).list(None, None)
if err:
    raise Exception(err)
...
result, err = client.OpenskyNetwork(None).load(
    {"id": "test01"}, None
)
