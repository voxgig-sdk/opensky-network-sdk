...
class TestFlightDirect:

    def test_should_direct_list_flight(self):
        setup = _flight_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-flight", "live" if setup["live"] else "unit")
        if _skip:
...
def _flight_direct_setup(mockres):
