# OpenskyNetwork SDK test runner
...
class OpenskyNetworkTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            OpenskyNetworkTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = OpenskyNetworkTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = OpenskyNetworkTestRunner.getenv("OPENSKY_NETWORK_TEST_LIVE")
        override = OpenskyNetworkTestRunner.getenv("OPENSKY_NETWORK_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = OpenskyNetworkTestRunner.getenv(key)
        ...
        explain = OpenskyNetworkTestRunner.getenv("OPENSKY_NETWORK_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["OPENSKY_NETWORK_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if OpenskyNetworkTestRunner._test_control is not None:
            return OpenskyNetworkTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                OpenskyNetworkTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            OpenskyNetworkTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = OpenskyNetworkTestRunner.load_test_control()
...
def load_env_local():
    OpenskyNetworkTestRunner.load_env_local()


def env_override(m):
    return OpenskyNetworkTestRunner.env_override(m)


def entity_data(v):
    return OpenskyNetworkTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return OpenskyNetworkTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return OpenskyNetworkTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return OpenskyNetworkTestRunner.load_test_control()


def live_client_options():
    return OpenskyNetworkTestRunner.live_client_options()


def live_delay_ms():
    return OpenskyNetworkTestRunner.live_delay_ms()
