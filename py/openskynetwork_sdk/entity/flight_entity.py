...
from openskynetwork_sdk.openskynetwork_types import (
    Flight,
    FlightListMatch,
)
...
class FlightEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return FlightEntity(self._client, opts)
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Flight]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Flight().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
