...
//     (\`new OpenskyNetworkSDK(...)\` / \`OpenskyNetworkSDK.test(...)\` become
//     \`OpenskyNetworkSDK.test({ entity: ... })\`), transpiled to JS, and run
...
// rewrite real client construction (\`new OpenskyNetworkSDK(...)\` and any
// \`OpenskyNetworkSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+OpenskyNetworkSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/OpenskyNetworkSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
