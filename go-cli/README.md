...
boru-driven command-line client **and** interactive REPL for the OpenskyNetwork
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/opensky-network-cli)
make build

# 2. See usage (words, entities, env vars)
./opensky-network-cli --help
...
./opensky-network-cli
opensky-network> list flight
opensky-network> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/opensky-network-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/opensky-network-cli list flight
   ```
...
Run with no arguments to open a REPL (prompt `opensky-network>`). Each line is
evaluated as its own boru expression:

```text
$ ./opensky-network-cli
opensky-network> list flight
opensky-network> /help
opensky-network> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `flight`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/opensky-network-cli`. |
...
(`./opensky-network-cli <expr>`) and interactively (the REPL), and expressions compose the
