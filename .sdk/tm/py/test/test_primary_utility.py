...
from projectname_sdk import OpenskyNetworkSDK
from projectname_sdk.core.spec import OpenskyNetworkSpec
from projectname_sdk.core.result import OpenskyNetworkResult
from projectname_sdk.core.response import OpenskyNetworkResponse
from projectname_sdk.core.operation import OpenskyNetworkOperation
from projectname_sdk.core.error import OpenskyNetworkError
from projectname_sdk.core import helpers
from projectname_sdk.feature.base_feature import OpenskyNetworkBaseFeature
...
def _err_from_map(m):
    ...
    if msg == "":
        return None
    code = m.get("code", "")
    return OpenskyNetworkError(code, msg)
...
class TestPrimaryUtility:
    ...
    def test_make_error_no_throw(self):
        ...
        ctx.result = OpenskyNetworkResult({
            "ok": False,
            "resdata": {"id": "safe01"},
        })
    ...
    def test_feature_init_basic(self):
        init_client = OpenskyNetworkSDK.test(None, None)
        ...
        class TestInitFeature(OpenskyNetworkBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "initfeat"
                self.active = True
    ...
    def test_feature_init_inactive(self):
        ...
        class TestInitFeatureInactive(OpenskyNetworkBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "nofeat"
                self.active = False
    ...
    def test_fetcher_blocked_test_mode(self):
        ...
        blocked_client = OpenskyNetworkSDK({
            "base": "http://localhost:8080",
            "system": {
                "fetch": mock_fetch,
            },
    ...
    def test_make_result_no_result(self):
        ...
        utility = client._utility
        ctx = _make_test_full_ctx(client, utility)
        ctx.spec = OpenskyNetworkSpec({"step": "start"})
        ctx.result = None
    ...
    def test_make_spec_basic(self):
        ...
        spec_client = OpenskyNetworkSDK.test(None, setup_opts)
    ...
    def test_operator_basic(self):
        ...
        def subject(entry):
            ...
            if not isinstance(in_val, dict):
                in_val = {}
            op = OpenskyNetworkOperation(in_val)
            return {
                "entity": op.entity,
                "name": op.name,
                "input": op.input,
    ...
    def test_prepare_auth_basic(self):
        ...
        auth_client = OpenskyNetworkSDK.test(None, setup_opts)
