...
// OpenskyNetwork SDK retry feature
...
class OpenskyNetworkRetryFeature extends OpenskyNetworkBaseFeature
{
    ...
    public function init(OpenskyNetworkContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (OpenskyNetworkContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(OpenskyNetworkContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(OpenskyNetworkContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
