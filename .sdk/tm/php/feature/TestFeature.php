...
// OpenskyNetwork SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class OpenskyNetworkTestFeature extends OpenskyNetworkBaseFeature
{
    ...
    public function init(OpenskyNetworkContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (OpenskyNetworkContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = OpenskyNetworkParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (OpenskyNetworkContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(OpenskyNetworkContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = OpenskyNetworkParam::call($ctx, $k);
