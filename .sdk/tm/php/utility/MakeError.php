...
class OpenskyNetworkMakeError
{
    public static function call(?OpenskyNetworkContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new OpenskyNetworkContext([], null);
        }
        $op = $ctx->op ?? new OpenskyNetworkOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new OpenskyNetworkResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof OpenskyNetworkError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new OpenskyNetworkError('', $msg, $ctx);
        ...
        if ($err instanceof OpenskyNetworkError) {
