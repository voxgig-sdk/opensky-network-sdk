...
// OpenskyNetwork SDK feature test
//
...
class FtCtrl extends OpenskyNetworkControl
{
...
class FtClient
{
    ...
    private OpenskyNetworkUtility $utility;

    public function __construct(OpenskyNetworkUtility $utility, array $options)
    {
    ...
    public function get_utility(): OpenskyNetworkUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (OpenskyNetworkContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public OpenskyNetworkContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (OpenskyNetworkContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = OpenskyNetworkConfig::make_feature($name);
    ...
    private static function buildUrl(OpenskyNetworkSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof OpenskyNetworkError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new OpenskyNetworkResponse($fetched);
        }
    ...
    private function fail(OpenskyNetworkContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (OpenskyNetworkContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
