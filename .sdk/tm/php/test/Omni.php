...
//    only - a typed OpenskyNetworkContext there would make every ctx
...
final class OpenskyNetworkOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new OpenskyNetworkContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new OpenskyNetworkSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new OpenskyNetworkResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new OpenskyNetworkError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new OpenskyNetworkResponse($clean['response']);
    ...
    public static function err_from_map($m): ?OpenskyNetworkError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new OpenskyNetworkError(is_string($code) ? $code : '', $msg);
    }
