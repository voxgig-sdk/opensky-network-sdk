...
// OpenskyNetwork SDK pipeline test
//
...
{
    private static function utility(): OpenskyNetworkUtility
    {
        return new OpenskyNetworkUtility();
    }
    ...
    private static function ctx(array $over = []): OpenskyNetworkContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new OpenskyNetworkContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new OpenskyNetworkOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof OpenskyNetworkError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new OpenskyNetworkOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = OpenskyNetworkMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new OpenskyNetworkOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = OpenskyNetworkMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new OpenskyNetworkOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = OpenskyNetworkMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new OpenskyNetworkSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = OpenskyNetworkMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new OpenskyNetworkResponse([]),
            'result' => new OpenskyNetworkResult([]),
        ]);
        [, $err] = OpenskyNetworkMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new OpenskyNetworkSpec([]),
            'response' => null,
            'result' => new OpenskyNetworkResult([]),
        ]);
        [, $err] = OpenskyNetworkMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new OpenskyNetworkSpec(['step' => 's']),
            'response' => new OpenskyNetworkResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new OpenskyNetworkResult([]),
        ]);
        [, $err] = OpenskyNetworkMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new OpenskyNetworkSpec(['step' => 's']),
            'response' => new OpenskyNetworkResponse(self::resp(200, ['v' => 1])),
            'result' => new OpenskyNetworkResult([]),
        ]);
        [, $err] = OpenskyNetworkMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new OpenskyNetworkSpec(['step' => 's']),
            'response' => new OpenskyNetworkResponse(self::resp(200, ['v' => 2])),
            'result' => new OpenskyNetworkResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        OpenskyNetworkMakeResponse::call($ctx);
    ...
    {
        $preset = new OpenskyNetworkResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new OpenskyNetworkSpec([]),
            'response' => new OpenskyNetworkResponse([]),
            'result' => new OpenskyNetworkResult([]),
        ...
        [$got, $err] = OpenskyNetworkMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new OpenskyNetworkResult([])]);
        [, $err] = OpenskyNetworkMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new OpenskyNetworkSpec([]), 'result' => null]);
        [, $err] = OpenskyNetworkMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new OpenskyNetworkSpec(['step' => 's']),
            'result' => new OpenskyNetworkResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new OpenskyNetworkOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = OpenskyNetworkMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new OpenskyNetworkSpec(['step' => 's']),
            'result' => new OpenskyNetworkResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new OpenskyNetworkOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = OpenskyNetworkMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new OpenskyNetworkResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new OpenskyNetworkSpec([]),
            'result' => new OpenskyNetworkResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = OpenskyNetworkMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = OpenskyNetworkMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new OpenskyNetworkError('boom', 'boom');
        $utility->fetcher = function (OpenskyNetworkContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new OpenskyNetworkSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = OpenskyNetworkMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (OpenskyNetworkContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = OpenskyNetworkMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(OpenskyNetworkResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        OpenskyNetworkMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (OpenskyNetworkContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new OpenskyNetworkSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = OpenskyNetworkMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new OpenskyNetworkResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new OpenskyNetworkSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = OpenskyNetworkMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = OpenskyNetworkMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = OpenskyNetworkMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new OpenskyNetworkResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, OpenskyNetworkDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new OpenskyNetworkResult(['ok' => false])]);
        $this->expectException(OpenskyNetworkError::class);
        OpenskyNetworkDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new OpenskyNetworkResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', OpenskyNetworkMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        OpenskyNetworkMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new OpenskyNetworkBaseFeature();
        $b = new OpenskyNetworkBaseFeature();
        OpenskyNetworkFeatureAdd::call($ctx, $a);
        OpenskyNetworkFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): OpenskyNetworkBaseFeature
    {
        $f = new OpenskyNetworkBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        OpenskyNetworkFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): OpenskyNetworkContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new OpenskyNetworkSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = OpenskyNetworkPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        OpenskyNetworkPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new OpenskyNetworkResponse(['headers' => null]),
            'result' => new OpenskyNetworkResult([]),
        ]);
        OpenskyNetworkResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        OpenskyNetworkResultBody::call($ctx);
